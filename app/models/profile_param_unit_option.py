...
class ProfileParamUnitOption(models.Model):
    ...
    def get_unit_option(cls, profile, parameter):
        try:
            return cls.objects.get(profile=profile, parameter=parameter).unit_option
...
def profile_param_unit_option_post_delete(sender, instance, *args, **kwargs):
    """ To clean up any redundant model instances, e.g. no DataPoint instances pointing to a Parameter that such a model
     points to. Note that as sender argument not included in the decorator, this callback is called for all models """
    from app.models.data_point import DataPoint
    if sender == DataPoint:
        filter_kwargs = dict(profile=instance.profile, parameter=instance.parameter)
